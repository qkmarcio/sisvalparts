...
class PHPJasperXML {
    ...
    $this->cndriver=$cndriver;
		...
		elseif($cndriver=="ibase"){
			global $ibport;
			if($ibport=="" || $ibport==0)
                $ibport=3050;
			$conn_string=$this->db_host.'/'.$ibport.':'.$this->db_or_dsn_name;
            $this->myconn = ibase_connect($conn_string, $this->db_user, $this->db_pass);
...
//        $b=$data->subreportParameter;
    ...
    public function transferDBtoArray($host,$user,$password,$db_or_dsn_name,$cndriver="mysql") {
		...
		elseif($cndriver=="ibase"){
			$result= ibase_query ($this->myconn, $this->sql);
			while ($row = ibase_fetch_object($result)) {
                foreach($this->arrayfield as $out) {
					
					$index=strtoupper($out);
					
                    $this->arraysqltable[$this->m]["$out"]=$row->$index;
                }
                $this->m++;
            }
